...
class VNitem(object):
    def __init__(self, surface, s_size, t_pos, t_size=textbox_size, t_rgb=textbox_rgb, t_alpha=textbox_alpha, \
        ...
        self.t_pos = t_pos
        self.settings = self.init_setting_buttons()
    ...
    def init_setting_buttons(self):
        settings = {}
        settings['save'] = Button((778,375),(50,30),(0,0,0),180,'save','./hksn.ttf',15)
        settings['load'] = Button((778,415),(50,30),(0,0,0),180,'load','./hksn.ttf',15)
        return settings
    ...
    def update_image(self, name, pos=(0,0), size=None, colorkey=None):
        ...
        if not size:
            self.settings['save'].render(self.surface)
            self.settings['load'].render(self.surface)
