...
let reservedWords = [
  ...
  ("image", fun i -> Parser.IMAGE i);
  ("music", fun i -> Parser.MUSIC i);
  ("silent", fun i -> Parser.SILENT i);
  ("halt", fun i -> Parser.HALT i);
  ("nop", fun i -> Parser.NOP i);
  ("show", fun i -> Parser.SHOW i);
  ("hide", fun i -> Parser.HIDE i);
  ("play", fun i -> Parser.PLAY i);
  ("say", fun i -> Parser.SAY i);
  ("menu", fun i -> Parser.MENU i);
  ("character", fun i -> Parser.CHARACTER i);
  ("nobody", fun i -> Parser.NOBODY i);
  ("scene", fun i -> Parser.SCENE i);
  ("blank", fun i -> Parser.BLANK i);
  ("main", fun i -> Parser.MAIN i);
...
| "```" _* "```"
    { Parser.HEADER{i=info lexbuf; v=text lexbuf} }

| "left" | "right" | "top" | "bottom" | "topleft" | "bottomleft" | "topright"
| "bottomright" | "medium" | "custom1" | "custom2" | "custom3" | "custom4"
    { Parser.PARAM{i=info lexbuf; v=text lexbuf} }
...
| "\"" { resetStr(); startLex := info lexbuf; addStr '"'; string lexbuf }
...
and string = parse
  '"'  { addStr '"'; Parser.STRINGV {i = !startLex; v=getStr()} }
